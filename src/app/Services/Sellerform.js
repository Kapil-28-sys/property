...
import { createSellerProperty } from "../../servicesapi/sellerformapi"; // adjust path if needed

/* ─────────────────────────────────────
   SELLER FORM — FIXED + REDESIGNED
───────────────────────────────────── */
export default function SellerForm() {
  ...
  /* ── Active section for animated tab nav ── */
  const [activeSection, setActiveSection] = useState(0);

  const formRef = useRef(null);

  /* ── Load countries & default states ── */
  useEffect(() => {
  ...
  const uploadImages = async () => {
    ...
    imgs.forEach((img) => fd.append("files", img));
    const res = await fetch("/api/upload", { method: "POST", body: fd });
    if (!res.ok) {
      let message = "Image upload failed";
      try { const j = await res.json(); message = j.message || message; } catch (_) {}
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setFormState({ loading: false, status: "ok", msg: `Listed successfully! · Seller #${json.seller?.id ?? "—"}` });
      ...
      setFormState({ loading: false, status: "err", msg: err.message || "Something went wrong. Please try again." });
    }
  };
        ...
        <nav style={S.nav} aria-label="Form sections">
          {sections.map((s, i) => (
            <button
              ...
              style={{
                ...S.navBtn,
                ...(activeSection === i ? S.navBtnActive : {}),
              }}
            >
              {s.icon}
              <span>{s.label}</span>
          ...
          <form ref={formRef} onSubmit={submit}>
            ...
            <Section show={activeSection === 0} title="Contact Information">
              <Field label="Full Name" required>
                <input style={S.input} name="name" placeholder="John Doe" value={fields.name} onChange={set("name")} required />
              </Field>
              <Field label="Phone Number" required>
                <input style={S.input} name="phone" placeholder="+91 98765 43210" value={fields.phone} onChange={set("phone")} required />
              </Field>
              <Field label="Email Address" span required>
                <input style={S.input} name="email" type="email" placeholder="john@email.com" value={fields.email} onChange={set("email")} required />
              </Field>
            ...
            <Section show={activeSection === 1} title="Location Details">
              <Field label="Country">
                <select style={S.input} name="country" value={selectedCountry} onChange={handleCountryChange}>
                  {countries.map((c) => <option key={c.isoCode} value={c.isoCode}>{c.name}</option>)}
                </select>
              </Field>
              <Field label="State">
                <select style={S.input} name="state" value={selectedState} onChange={handleStateChange}>
                  <option value="">Select State</option>
                  {states.map((s) => <option key={s.isoCode} value={s.isoCode}>{s.name}</option>)}
                </select>
              </Field>
              <Field label="City">
                <select style={S.input} name="city" value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)}>
                  <option value="">Select City</option>
                  {cities.map((c) => <option key={c.name} value={c.name}>{c.name}</option>)}
                </select>
              </Field>
              <Field label="Area / Locality">
                <input style={S.input} name="area" placeholder="Sector 17, MG Road…" value={fields.area} onChange={set("area")} />
              </Field>
              <Field label="Pincode">
                <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                  <input style={{ ...S.input, paddingRight: 130 }} name="pincode" placeholder="110001" value={fields.pincode} onChange={set("pincode")} />
              ...
              <Field label="Full Address" span>
                <textarea style={S.textarea} name="address" rows={2} placeholder="House No., Street, Landmark…" value={fields.address} onChange={set("address")} />
              </Field>
            ...
            <Section show={activeSection === 2} title="Property Details">
              <Field label="Property Type" required>
                <select style={S.input} name="propertyType" value={fields.propertyType} onChange={set("propertyType")} required>
                  <option value="">Select type</option>
                  {["Apartment","Villa","Plot","Commercial"].map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Furnished Status">
                <select style={S.input} name="furnishedType" value={fields.furnishedType} onChange={set("furnishedType")}>
                  <option value="">Select status</option>
                  {["Furnished","Semi Furnished","Unfurnished"].map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Bedrooms">
                <input style={S.input} name="bedrooms" type="number" min="0" placeholder="3" value={fields.bedrooms} onChange={set("bedrooms")} />
              </Field>
              <Field label="Bathrooms">
                <input style={S.input} name="bathrooms" type="number" min="0" placeholder="2" value={fields.bathrooms} onChange={set("bathrooms")} />
              </Field>
              <Field label="Area Size">
                <input style={S.input} name="areaSize" type="number" min="0" placeholder="1200" value={fields.areaSize} onChange={set("areaSize")} />
              </Field>
              <Field label="Area Unit">
                <select style={S.input} name="areaUnit" value={fields.areaUnit} onChange={set("areaUnit")}>
              ...
              <Field label="Expected Price (₹)" span required>
                <div style={S.priceWrap}>
                  ...
                  <input style={{ ...S.input, paddingLeft: 36 }} name="price" placeholder="45,00,000" value={fields.price} onChange={set("price")} required />
                </div>
              </Field>
            ...
            <Section show={activeSection === 3} title="Description & Images">
              <Field label="Property Description" span>
                <textarea style={{ ...S.textarea, minHeight: 100 }} name="description" rows={4} placeholder="Highlight key features, nearby schools, metro, amenities…" value={fields.description} onChange={set("description")} />
              </Field>
              ...
              <div style={{ gridColumn: "1/-1", display: "flex", gap: 12, marginTop: 4 }}>
                ...
                >
                  ...
                  {formState.loading ? "Submitting…" : "Publish Listing →"}
                </button>
              </div>
            </Section>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus, textarea:focus, select:focus {
          outline: none;
          box-shadow: 0 0 0 2px rgba(200,164,90,0.35);
...
/* ── Section wrapper (conditionally rendered) ── */
...
function Field({ label, children, span, required }) {
  return (
    <div style={{ ...(span ? { gridColumn: "1/-1" } : {}) }}>
      <label style={S.label}>
        {label}
        {required && <span style={{ color: "#c8a45a", marginLeft: 3 }}>*</span>}
      </label>
...
function NavFooter({ onBack, onNext }) {
  return (
    <div style={{ gridColumn: "1/-1", display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 4 }}>
      ...
      {onNext && (
        <button type="button" onClick={onNext} style={S.nextBtn}>Continue →</button>
      )}
    </div>
  );
}
...
const S = {
  ...
  bgGrid: {
    ...
    inset: 0,
    backgroundImage: `
      linear-gradient(rgba(200,164,90,0.07) 1px, transparent 1px),
      linear-gradient(90deg, rgba(200,164,90,0.07) 1px, transparent 1px)
    `,
  ...
  submitBtn: {
    ...
    gap: 10,
    transition: "opacity .2s",
    boxShadow: "0 6px 24px rgba(200,164,90,0.4)",
    letterSpacing: "0.02em",
  },
};